...
export default function InboxPage() {
  ...
  useEffect(() => {
    async function fetchMessages() {
      // 1. جلب بيانات المستخدم الحالي المفرزة بصلاحياته
      ...
      // 2. جلب رتبة المستخدم (الأدمن يرى كل الرسائل، البائع يرى رسائله فقط)
      ...
      if (profile?.role !== 'admin') {
        // البائع يرى الرسائل الموجهة إليه (كمستقبل) أو التي أرسلها (كمرسل)
        ...
        <div className="space-y-4">
          ...
          {messages.length === 0 && (
            <div className="text-center py-16 bg-white rounded-2xl border text-gray-400">
              ...
              <p className="text-xs mt-1">عندما يضغط الزوار على زر "التفاصيل" ويرسلون رسالة، ستظهر هنا فوراً.</p>
            </div>
          )}
        </div>
